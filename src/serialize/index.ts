...
const $unsupported_object = new $('__UNSUPPORTED_OBJECT__');
const $function = new $('__FUNCTION__');
const $date = new $('__DATE__');
const $regexp = new $('__REGEXP__');
    ...
    switch (typeof raw) {
        ...
        case 'object':
            ...
            if (raw instanceof Map) {
                ...
                return $map.wrap(JSON.stringify(serialize(obj)));
            }
            if (raw instanceof Set) {
                const array = Array.from(raw).sort((a, b) => {
                    return serialize(a).localeCompare(serialize(b));
                });
                return $set.wrap(JSON.stringify(serialize(array)));
            }
            ...
            if (raw instanceof RegExp) {
                return $regexp.wrap(JSON.stringify(raw.source));
            }
            if (isPOJO(raw)) {
            ...
            return $unsupported_object.wrap(JSON.stringify(placeholder));
        case 'function':
...
export function deserialize<T>(cooked: string): T {
    ...
    if ($map.validate(cooked)) {
        ...
        const parsed = JSON.parse(unwrapped);
        const object = deserialize<Record<string, string>>(parsed);
        const entries = Object.entries(object).map(([key, value]) => {
            return [deserialize(key), deserialize(value)] as const;
        });
    ...
    if ($set.validate(cooked)) {
        ...
        const parsed = JSON.parse(unwrapped);
        const array = deserialize(parsed) as unknown[];
